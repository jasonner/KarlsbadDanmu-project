  ...
  <div class="sedMsg-page">
    ...
    <div class="header-box">
      <div class="header-left">
        <img src="@/assets/img/sedMsg/logo-left.png" alt="" srcset="" />
      </div>
      <div class="header-right">
        <img src="@/assets/img/sedMsg/logo-right.png" alt="" srcset="" />
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    initWXlogin(key) {
      let that = this;
      that.axios.get("https://api.lehuitech.com.cn/WXAuthorization/GetUserWXInfo?callkey=" + key).then(function (res) {
        console.log(res);
        if (res.status == 200) {
          if (!res.data.user) {
            window.location.href = "https://api.lehuitech.com.cn/WXAuthorization/WXlogin?callkey=" + key+'&state='+'https%3A%2F%2Fbsd2021.lehuitech.com.cn%2Fdanmu%2Findex.html%23%2FsedMsg';
          } else {
            that.sedOpenid(res.data.user);
            that.getKey();
          }
        } else {
          that.$message({
    ...
    sedOpenid(user){
      ...
      that.axios.post("danmu/AddWXInfo", qs.stringify(promise)).then(function (res) {
        console.log(res);
        if (res.status == 200) {
    ...
    getKey() {
      let that = this;
      that.axios.get("https://api.lehuitech.com.cn/WXAuthorization/jsapi?url=" + window.location.href).then(function (res) {
        console.log(res);
        if (res.status == 200) {
          that.initIMg(res.data);
        } else {
...
.sedMsg-page {
  width: 100%;
  height: 100%;
  background: url(../assets/img/sedMsg/bg.png) center no-repeat;
